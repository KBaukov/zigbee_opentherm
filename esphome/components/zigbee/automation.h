...
class ZigBeeConnectTrigger : public Trigger<> {
 public:
  explicit ZigBeeConnectTrigger(ZigBeeComponent *parent) {
...
template<typename Ts> class ZigBeeOnValueTrigger : public Trigger<Ts>, public Component {
 ...
 protected:
  void on_value_(esp_zb_zcl_attribute_t attribute) {
    ...
    } else if ( attribute.data.value ) {
      this->trigger(get_value_by_type<Ts>(0, attribute.data.value));
    }
  }
  ZigBeeAttribute *parent_;
};
