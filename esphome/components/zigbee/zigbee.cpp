    ...
    case ESP_ZB_BDB_SIGNAL_DEVICE_REBOOT:
      ...
      if (err_status == ESP_OK) {
        ...
        if (esp_zb_bdb_is_factory_new()) {
		  ...
		  zigbeeC->on_connect_callback_.call();
        }
      } else {
...
static esp_err_t zb_attribute_handler(const esp_zb_zcl_set_attr_value_message_t *message) {
  ...
  //int* val = (int*)message->attribute.data.value;
  ESP_LOGD(TAG, "Received message: endpoint(%d), cluster(0x%x), attribute(0x%x), data size(%d)",
           message->info.dst_endpoint, message->info.cluster, message->attribute.id, message->attribute.data.size); //, *(int*)message->attribute.data.value);
